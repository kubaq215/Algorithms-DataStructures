...
private:
    ...
    class vertex { public: string label; };
...
{
    return outedge_iter(g->size, k, g);
}
...
int main()
{
    Graph g(5);
    int po = 0;
    auto j = g.vertices_begin();
    auto n = g.vertices_end();
    Graph::vertex_iter t(4, &g);
    while(j!=n)
    {
        (*g.connect(j, t)).weight = po++;
        ++j;
    }
    auto i = t.outedges_begin();
    ++i;
    // ++i;
    // ++i;
    // ++i;
    // ++i;
    auto e = t.outedges_end();
    cout << g.edge_count();
    while(i!=e)
    {
        cout << (*i).weight;
